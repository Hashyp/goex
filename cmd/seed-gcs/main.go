...
func main() {
	...
	if err := seed(ctx, client, cfg.ProjectID, seedData); err != nil {
		log.Fatal(err)
	}

	log.Println("GCS seed complete")
}

func seed(ctx context.Context, client *storage.Client, projectID string, data []seedObject) error {
	...
	for _, bucketName := range buckets {
		if err := gcsblob.EnsureBucket(ctx, client, projectID, bucketName); err != nil {
	...
	for idx, item := range data {
		writer := client.Bucket(item.bucket).Object(item.key).NewWriter(ctx)
